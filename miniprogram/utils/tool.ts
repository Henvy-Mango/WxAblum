// Date格式化时间
export const formatTime = (date: Date, fmt = 'yyyy-MM-dd') => {
  const o = {
...
// 一维数组转二维数组
export const listToMatrix = (list: any[], elementsPerSubArray: number, truncated: number) => {
  const matrix = [];
  let i;
  let col;
  let row;
...
// 快速排序，时间降序排列
export const qSort = (arr: any[]): any[] => {
  if (arr.length <= 1) {
    return arr;
  }
...
// 选中文件之后，计算一个随机的短文件名
export const getRandFileName = (filePath: string) => {
